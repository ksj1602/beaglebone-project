...
void Write(int fd, void* buffer, int bytes_to_be_written)
{
    int bytes_written = 0;
    int tmp = 0;
    while ((tmp = write(fd, buffer + bytes_written, bytes_to_be_written - bytes_written)) && (bytes_written < bytes_to_be_written))
    {
        if (tmp == -1)
...
void parse_commands()
{
    ...
    // Clear out input buffer after all commands processed
...
int main (int argc, char *argv[])
{
    ...
    Write(socket_fd, socket_send_buffer, strlen(socket_send_buffer));
    ...
    while (true)
    {
        ...
        {
            ret = read(socket_fd, socket_receive_buffer, BUFFERSIZE);
            if (ret)
            {
                if (ret == -1)
                    exit_with_code("Error in reading from socket", 2);
                parse_commands();
            }
        }
